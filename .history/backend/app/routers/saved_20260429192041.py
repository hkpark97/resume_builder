...
from app.schemas import SaveAnalysisRequest, SavedAnalysisResponse, FeedbackRequest
...
def save_analysis(payload: SaveAnalysisRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    row = MatchAnalysis(user_id=current_user.id, title=payload.title, company=payload.company, final_score=payload.final_score, report_markdown=payload.report_markdown, structured_report=payload.structured_report, resume_text=payload.resume_text, job_description=payload.job_description)
    db.add(row); db.commit(); db.refresh(row)
    return SavedAnalysisResponse(id=row.id, title=row.title, company=row.company, final_score=row.final_score, report_markdown=row.report_markdown)
